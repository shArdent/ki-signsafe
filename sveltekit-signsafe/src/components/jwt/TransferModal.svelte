<script lang="ts">
	...
	async function fetchUsers(query: string): Promise<void> {
		...
		try {
			const { data } = await apiJWT.get(`/users?name=${encodeURIComponent(query)}`);

			const filtered = data.filter((e: User) => e.id !== currentID);
	...
	async function handleTransfer(): Promise<void> {
		...
		try {
			const data = await apiJWT.post('/transfer', payload);
